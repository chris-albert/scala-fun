...
import io.lbert.iot.Device.{ReadTemperature, RecordTemperature, RespondTemperature, TemperatureRecorded}
import io.lbert.iot.DeviceGroup.{ReplyDeviceList, RequestAllTemperatures, RequestDeviceList}
import io.lbert.iot.DeviceManager.{DeviceRegistered, RequestTrackDevice}
...
class Device(groupId: String, deviceId: String) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case RequestTrackDevice(groupId, deviceId) =>
      log.warning(
        "Ignoring TrackDevice request for {}-{}. This actor is responsible fro {}-{}",
...
class DeviceGroup(groupId: String) extends Actor with ActorLogging {
  ...
  var actorToDevice = Map.empty[ActorRef, String]
  ...
  override def receive: Receive = {
    case trackMsg @ RequestTrackDevice(`groupId`,_) =>
      deviceIdToActor.get(trackMsg.deviceId) match {
        ...
        case None =>
          ...
          actorToDevice += deviceActor -> trackMsg.deviceId
          deviceActor forward trackMsg
      }
    case RequestTrackDevice(groupId, deviceId) =>
      log.warning(
    ...
    case Terminated(deviceActor) =>
      val deviceId = actorToDevice(deviceActor)
      log.info("Device actor for {} has been terminated", deviceId)
      actorToDevice -= deviceActor
      deviceIdToActor -= deviceId
    case RequestAllTemperatures(requestId) =>
      context.actorOf(DeviceGroupQuery.props(
        actorToDeviceId = actorToDevice,
        requestId = requestId,
        requester = sender(),
        timeout = 3.seconds
      ))
